...
public:
	...
	void generate()
	{
		...
		//Value computed in BGR format, assuming B:c==0, G:c==1, R:c==2
		//Expr value = 0.0722f*input(x, y, c==0)/*B*/ + 0.7152f*input(x, y, c==1)/*G*/ + 0.2126f*input(x, y, c==2)/*R*/;
		Expr bucket = clamp(cast<uint8_t>(value), 0, 95);

		output(r.x, bucket, 1) += cast<uint8_t>(10);
